...
export function Navbar() {
    ...
    return (
        <motion.nav
            initial={{ y: -100 }}
            animate={{ y: 0 }}
            className="fixed top-0 left-0 right-0 z-50 bg-[#F2F2F2]/80 backdrop-blur-md border-b border-black/5"
        >
            <div className="container mx-auto px-6 h-16 flex items-center justify-between">
                <Link href="/" className="text-sm font-black tracking-tighter uppercase shrink-0">
                    Editor de la Verdad
                </Link>

                <NavBar
                    items={navItems}
                    className="relative bottom-auto top-auto left-auto translate-x-0 z-auto mb-0 pt-0 sm:pt-0"
                />
